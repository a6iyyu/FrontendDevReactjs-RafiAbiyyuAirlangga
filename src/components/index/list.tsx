...
import { DataTidakDitemukan } from "./data-tidak-ditemukan";
...
export const DaftarRestoran: React.FC = () => {
  ...
  return (
    <main className="list mx-auto mb-20 mt-14 h-fit w-4/5 text-slate-50">
      ...
      <h3 className="mb-8 cursor-default rounded-xl text-4xl font-bold underline decoration-1">
        Daftar Restoran
      </h3>
      {filteredRestaurants.length === 0 ? (
        ...
        <section className="grid h-full w-full cursor-default grid-cols-1 place-items-center gap-10 sm:grid-cols-2 lg:grid-cols-3">
          {filteredRestaurants.map((resto) => (
            <article key={resto.id} id={resto.id} className="flex h-full w-full flex-col py-5">
